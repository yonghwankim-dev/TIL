...
다음 그림은 HTTPS에 의한 통신 과정입니다.

![img_5.png](img_5.png)

1. 클라이언트가 Client Hello 메시지를 송신하면서 SSL 통신을 시작합니다.

- 클라이언트가 제공하는 SSL 버전 지정됩니다.
- 암호 스위트(Cipher Suite)로 불리는 리스트(사용하는 암호화 알고리즘, 키 사이즈 등)가 포함됩니다.

2. 서버가 SSL 통신이 가능한 경우에 Server Hello 메시지로 응답합니다.

- SSL 버전, 암호 스위트가 포함됩니다.

3. 서버가 Certificate 메시지를 송신합니다.

- 메시지에는 공개키 증명서가 포함됩니다.

4. 서버가 Server Hello Done 메시지를 송신하여 최초의 SSL 네고시에이션 부분이 끝났음을 통지합니다.

5. SSL 네고시에이션이 끝나면 클라이언트는 Client Key Exchange 메시지로 응답합니다.

- 메시지에 통신을 암호화하는데 사용하는 Pre-Master secret이 포함됩니다.
- 이 메시지는 공개키 증명서에 꺼낸 공개키로 암호화되어 있습니다.

6. 클라이언트가 Change Cipher Spec 메시지를 송신합니다.

- 메시지의 의미는 이 메시지 이후의 통신은 암호키를 사용해서 통신하겠다는 의미입니다.

7. 클라이언트가 Finished 메시지를 송신합니다.

- 메시지에는 접속 전체의 체크 값을 포함합니다.
- 네고시에이션이 성공했는지 어떤지는 서버가 이 메시지를 올바르게 복호화할 수 있는지 아닌지가 결정됩니다.

8. 서버에서 Change Cipher Spec 메시지를 송신합니다.

9. 서버에서 Finished 메시지를 송신합니다.

10. 서버와 클라이언트간의 Finished 메시지 교환이 완료되면 SSL 접속이 확립하고 HTTP 통신합니다.

- 이후 통신은 SSL 통신으로 보호됩니다.
- 이후 통신은 HTTP 통신합니다.
...
- 접속 종료시 close_notify 메시지를 송신합니다.
- 이 후 TCP_FIN 메시지를 보내 TCP 통신을 종료합니다.

### SSL과 TLS

- HTTPS는 SSL과 TLS 프로토콜을 사용합니다.
...
- 통신 속도가 떨어집니다.
- CPU나 메모리 등의 리소스를 다량으로 소비합니다.

SSL 사용시 네트워크 부하

- HTTP 사용하는 것에 비해서 2~100배 정도 느려집니다.
- **TCP 접속과 HTTP 메시지 이외에 SSL 통신이 추가되어 전체적으로 처리해야할 통신이 증가하여 부하가 발생합니다.**
- SSL 엑셀레이터라는 하드웨어 서버를 사용해서 SSL만을 전용으로 처리하여 문제를 해결합니다.
