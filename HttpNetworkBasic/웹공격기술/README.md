...
- 공격자가 사용자의 세션 ID를 입수해서 악용하는 공격입니다.
- 세션 ID 입수 방법
    - 부적절한 생성 방법으로 세션 ID 추축
    - 도청이나 XSS 등에 의해서 세션 ID 도용
    - 세션 픽세이션 공격에 의한 세션 ID 강제

다음 그림은 웹 사이트의 XSS 취약성을 이용하여 세션 ID를 도용하여 공격자가 세션 ID를 인증하는 그림입니다.

![img_4.png](img_4.png)

## 세션 픽세이션

- 공격자가 지정한 세션 ID를 유저에게 강제적으로 사용하게 하는 공격입니다.
- 세션 고정 공격이라고 불립니다.
...
- 크로스 사이트 리퀘스트 포저리(CSRF, Cross-Site Request Forgeries)는 인증된 유저가 의도하지 않는 개인 정보나
  설정 정보 등을 공격자가 설치해 둔 함정에 의해 어떤 상태를 갱신하는 처리를 강제로 실행하는 공격
- 수동적 공격
- 크로스 사이트 리퀘스트 포저리 영향
    - 인증된 유저의 권한으로 설정 정보를 갱신
    - 인증된 유저의 권한으로 상품을 구입
    - 인즈오딘 유저의 권한으로 게시판에 글 작성

### 크로스 사이트 리퀘스트 포저리 공격 사례
...
- 패스워드 크래킹(Password Cracking)은 패스워드를 논리적으로 이끌어내서 인증하는 공격입니다.
- 패스워드 크래킹 방법
    - 무차별 대입 공격
    - 사전 공격
    - 암호화된 데이터로부터 평문을 도출하는 방법
        - 레인보우 테이블
        - 열쇠 입수
        - 암호 알고리즘의 취약성
...
- 비밀 번호 시스템에서 취할 수 있는 모든 패스워드 후보를 시험해서 인증하는 공격
- ex) 계좌 비밀번호 0000~9999

### 사전 공격

- 사전에 패스워드 후보를 준비하고 그것을 시험해서 인증하는 공격

### 레인보우 테이블

- 레인보우 테이블(Rainbow Table)은 평문과 그에 대응하는 해시 값으로 구성된 데이터베이스 테이블입니다.

### 열쇠 입수

패스워드의 데이터가 공통키 암호 등으로 암호화되어 있을 때 암호화에서 사용된 키를 어떤 방법으로 입수하여 복호화하는 것입니다.

### 암호 알고리즘의 취약성

웹 애플리케이션의 개발자가 독자적인 암호 알고리즘으로 구현한 경우에 검증되지 않은 취약성을 발견하여 복호화하는 것입니다.

## 클릭 재킹

- 클릭 재킹(Clickjacking)은 투명한 버튼이나 링크를 함정으로 사용할 웹 페이지에 넣어두고 유저에게 링크를 클릭하게 해서
  의도하지 않은 내용에 접근 시키는 공격입니다.

![img_7.png](img_7.png)

## Dos 공격

- Dos 공격(Denial of Service attack)은 서비스 제공을 정지 상태로 만드는 공격입니다.
- Dos 공격 방법
    - 접근을 집중시켜 부하를 걸어 리소스를 다 소비하게 해 사실상 서비스를 정지상태로 만듭니다.
    - 취약성을 공격해 서비스를 정지시킵니다.

## 백도어

- 백도어(Backdoor)는 제한된 기능을 정규 절차를 밟지 않고 이용하기 위해 설치된 뒷문입니다.
- 백도어 종류
    - 개발 단계에 디버그용으로 추가한 백도어
    - 개발자가 자기 자신의 이익을 위해 추가된 백도어
    - 공격자가 어떠한 방법을 사용해서 설치한 백도어
